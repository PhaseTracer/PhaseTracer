...
if __name__ == "__main__":
    ...
    for i, a in enumerate(ax):
        a.plot(lambda_hs, lower[:, i], color="red", ls="--", label="$Q = 1/2 m_t$")
        a.plot(lambda_hs, central[:, i], color="red", ls="-", label="$Q = m_t$")
        a.plot(lambda_hs, upper[:, i], color="red", ls=":", label="$Q = 2 m_t$")
