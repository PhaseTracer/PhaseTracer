...
if __name__ == "__main__":
    ...
    lower = transpose_list_dict([call_pt("h_bar_expansion", l, 0.5 * mtop, xi, tree_level_tadpoles, tree_ewsb) for l in lambda_hs])
    central = transpose_list_dict([call_pt("h_bar_expansion", l, mtop, xi, tree_level_tadpoles, tree_ewsb) for l in lambda_hs])
    upper = transpose_list_dict([call_pt("h_bar_expansion", l, 2. * mtop, xi, tree_level_tadpoles, tree_ewsb) for l in lambda_hs])

    # TC and gamma

    for i, n in enumerate(["TC", "gamma_HT"]):
    ...
    plt.suptitle("PRM method")
    plt.tight_layout()
    plt.savefig("scale_h_bar_expansion.pdf")
