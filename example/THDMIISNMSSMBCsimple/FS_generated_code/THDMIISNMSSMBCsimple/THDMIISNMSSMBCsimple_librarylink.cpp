...
namespace THDMIISNMSSMBCsimple_librarylink {
...
private:
   ...
   void flush() {
      ...
      while (std::getline(buffer, line)) {
         ...
         MLPutFunction(link, "FSTHDMIISNMSSMBCsimpleMessage", 1);
...
class THDMIISNMSSMBCsimple_spectrum {
public:
   virtual ~THDMIISNMSSMBCsimple_spectrum() = default;

   virtual void put_model_spectra(MLINK link) const = 0;

   virtual const Spectrum_generator_problems& get_problems() const = 0;
   virtual void fill_slha_io(THDMIISNMSSMBCsimple_slha_io&) const = 0;
   virtual double get_model_scale() const = 0;
   virtual const THDMIISNMSSMBCsimple_observables& get_observables() const = 0;

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const THDMIISNMSSMBCsimple_input_parameters&) = 0;
   virtual void calculate_model_observables(const softsusy::QedQcd&, const Physical_input&) = 0;
};

template <typename Solver_type>
class THDMIISNMSSMBCsimple_spectrum_impl : public THDMIISNMSSMBCsimple_spectrum
{
public:
   virtual ~THDMIISNMSSMBCsimple_spectrum_impl() = default;
   ...
   virtual void fill_slha_io(THDMIISNMSSMBCsimple_slha_io&) const override;
   virtual double get_model_scale() const override { return std::get<0>(models).get_scale(); }
   virtual const THDMIISNMSSMBCsimple_observables& get_observables() const override { return observables; }

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const THDMIISNMSSMBCsimple_input_parameters&) override;
   virtual void calculate_model_observables(const softsusy::QedQcd&, const Physical_input&) override;

private:
   std::tuple<THDMIISNMSSMBCsimple_slha<THDMIISNMSSMBCsimple<Solver_type>>> models{};        ///< running parameters and pole masses
   Spectrum_generator_problems problems{};   ///< spectrum generator problems
   THDMIISNMSSMBCsimple_scales scales{};              ///< scale information
   THDMIISNMSSMBCsimple_observables observables{};    ///< observables
};
...
public:
   ...
   void set_input_parameters(const THDMIISNMSSMBCsimple_input_parameters& input_) { input = input_; }
...
private:
   THDMIISNMSSMBCsimple_input_parameters input{};     ///< model input parameters
   ...
   std::unique_ptr<THDMIISNMSSMBCsimple_spectrum> spectrum{nullptr};  ///< spectrum information

   THDMIISNMSSMBCsimple_slha_io get_slha_io() const;
...
void Model_data::check_spectrum_pointer() const
{
   if (!spectrum) {
      throw SetupError("No spectrum generator set! "
                       "Did you run FSTHDMIISNMSSMBCsimpleCalculateSpectrum[]?");
   }
}
...
void Model_data::put_input_parameters(MLINK link) const
{
   ...
   MLPutRuleTo(link, INPUTPARAMETER(lambdaNMSSM), "lambdaNMSSM");
   MLPutRuleTo(link, INPUTPARAMETER(kappaNMSSM), "kappaNMSSM");
   MLPutRuleTo(link, INPUTPARAMETER(AlambdaNMSSM), "AlambdaNMSSM");
   MLPutRuleTo(link, INPUTPARAMETER(AkappaNMSSM), "AkappaNMSSM");
   MLPutRuleTo(link, INPUTPARAMETER(AtopNMSSM), "AtopNMSSM");
   MLPutRuleTo(link, INPUTPARAMETER(mstopL), "mstopL");
   MLPutRuleTo(link, INPUTPARAMETER(mstopR), "mstopR");
   MLPutRuleTo(link, INPUTPARAMETER(MEWSB), "MEWSB");
   MLPutRuleTo(link, INPUTPARAMETER(TanBeta), "TanBeta");
   MLPutRuleTo(link, INPUTPARAMETER(vSIN), "vSIN");
                ...
                const std::vector<std::string>& heads = {})
{
   for (std::size_t i = 0; i < flags.size(); i++) {
      if (flags[i]) {
         ...
         MLPutSymbol(link, THDMIISNMSSMBCsimple_info::particle_names[i].c_str());
      }
   }
}
...
THDMIISNMSSMBCsimple_slha_io Model_data::get_slha_io() const
{
   check_spectrum_pointer();
...
void put_spectrum(const THDMIISNMSSMBCsimple_slha<Model>& model, MLINK link)
{
   MLPutRule(link, THDMIISNMSSMBCsimple_info::model_name);
   MLPutFunction(link, "List", 65);
   ...
   MLPutRuleTo(link, MODELPARAMETER(Lambda8), "Lambda8");
   MLPutRuleTo(link, MODELPARAMETER(Yd), "Yd");
   MLPutRuleTo(link, MODELPARAMETER(Ye), "Ye");
   MLPutRuleTo(link, MODELPARAMETER(Yu), "Yu");
   MLPutRuleTo(link, MODELPARAMETER(M123), "M123");
   MLPutRuleTo(link, MODELPARAMETER(M5), "M5");
   MLPutRuleTo(link, MODELPARAMETER(M112), "M112");
   MLPutRuleTo(link, MODELPARAMETER(M222), "M222");
   MLPutRuleTo(link, MODELPARAMETER(M332), "M332");
...
template <typename Solver_type>
void THDMIISNMSSMBCsimple_spectrum_impl<Solver_type>::put_model_spectra(MLINK link) const
{
...
template <typename Solver_type>
void THDMIISNMSSMBCsimple_spectrum_impl<Solver_type>::calculate_spectrum(
   const Spectrum_generator_settings& settings,
   const SLHA_io::Modsel& modsel,
   const softsusy::QedQcd& qedqcd,
   const THDMIISNMSSMBCsimple_input_parameters& input
)
{
...
template <typename Solver_type>
void THDMIISNMSSMBCsimple_spectrum_impl<Solver_type>::calculate_model_observables(
...
template <typename Solver_type>
void THDMIISNMSSMBCsimple_spectrum_impl<Solver_type>::fill_slha_io(THDMIISNMSSMBCsimple_slha_io& slha_io) const
{
...
{
   check_spectrum_pointer();
   THDMIISNMSSMBCsimple_observables observables = spectrum->get_observables();
...
void Model_data::check_spectrum(MLINK link) const
{
   ...
   for (const auto& s: problems.get_problem_strings())
      put_message(link, "FSTHDMIISNMSSMBCsimpleCalculateSpectrum", "error", s);

   for (const auto& s: problems.get_warning_strings())
      put_message(link, "FSTHDMIISNMSSMBCsimpleCalculateSpectrum", "warning", s);
...
#ifdef ENABLE_TWO_SCALE_SOLVER
   case 1:
      spectrum.reset(new THDMIISNMSSMBCsimple_spectrum_impl<Two_scale>());
...
Model_data make_data(const Dynamic_array_view<Element_t>& pars)
{
   ...
   INPUTPARAMETER(lambdaNMSSM) = pars[c++];
   INPUTPARAMETER(kappaNMSSM) = pars[c++];
   INPUTPARAMETER(AlambdaNMSSM) = pars[c++];
   INPUTPARAMETER(AkappaNMSSM) = pars[c++];
   INPUTPARAMETER(AtopNMSSM) = pars[c++];
   INPUTPARAMETER(mstopL) = pars[c++];
   INPUTPARAMETER(mstopR) = pars[c++];
   INPUTPARAMETER(MEWSB) = pars[c++];
   INPUTPARAMETER(TanBeta) = pars[c++];
   INPUTPARAMETER(vSIN) = pars[c++];
...
DLLEXPORT int FSTHDMIISNMSSMBCsimpleGetSettings(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::THDMIISNMSSMBCsimple_librarylink;

   if (!check_number_of_args(link, 1, "FSTHDMIISNMSSMBCsimpleGetSettings"))
...
DLLEXPORT int FSTHDMIISNMSSMBCsimpleGetSMInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::THDMIISNMSSMBCsimple_librarylink;

   if (!check_number_of_args(link, 1, "FSTHDMIISNMSSMBCsimpleGetSMInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSTHDMIISNMSSMBCsimpleGetInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::THDMIISNMSSMBCsimple_librarylink;

   if (!check_number_of_args(link, 1, "FSTHDMIISNMSSMBCsimpleGetInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSTHDMIISNMSSMBCsimpleOpenHandle(
...
DLLEXPORT int FSTHDMIISNMSSMBCsimpleCloseHandle(
   WolframLibraryData /* libData */, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy::THDMIISNMSSMBCsimple_librarylink;
...
DLLEXPORT int FSTHDMIISNMSSMBCsimpleSet(
   WolframLibraryData libData, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy;
   using namespace flexiblesusy::THDMIISNMSSMBCsimple_librarylink;
...
DLLEXPORT int FSTHDMIISNMSSMBCsimpleGetProblems(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::THDMIISNMSSMBCsimple_librarylink;

   if (!check_number_of_args(link, 1, "FSTHDMIISNMSSMBCsimpleGetProblems"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSTHDMIISNMSSMBCsimpleToSLHA(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::THDMIISNMSSMBCsimple_librarylink;

   if (!check_number_of_args(link, 1, "FSTHDMIISNMSSMBCsimpleToSLHA"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSTHDMIISNMSSMBCsimpleGetWarnings(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::THDMIISNMSSMBCsimple_librarylink;

   if (!check_number_of_args(link, 1, "FSTHDMIISNMSSMBCsimpleGetWarnings"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSTHDMIISNMSSMBCsimpleCalculateSpectrum(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::THDMIISNMSSMBCsimple_librarylink;

   if (!check_number_of_args(link, 1, "FSTHDMIISNMSSMBCsimpleCalculateSpectrum"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSTHDMIISNMSSMBCsimpleCalculateObservables(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::THDMIISNMSSMBCsimple_librarylink;

   if (!check_number_of_args(link, 1, "FSTHDMIISNMSSMBCsimpleCalculateObservables"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSTHDMIISNMSSMBCsimpleCalculateObservables", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSTHDMIISNMSSMBCsimpleCalculateSpectrum[]?");
      }
      ...
      put_message(link, "FSTHDMIISNMSSMBCsimpleCalculateObservables", "error", e.what_detailed());
      put_error_output(link);
   }
