...
namespace ScalarSingletZ2DMEFTHiggs_librarylink {
...
private:
   ...
   void flush() {
      ...
      while (std::getline(buffer, line)) {
         ...
         MLPutFunction(link, "FSScalarSingletZ2DMEFTHiggsMessage", 1);
...
class ScalarSingletZ2DMEFTHiggs_spectrum {
public:
   virtual ~ScalarSingletZ2DMEFTHiggs_spectrum() = default;

   virtual void put_model_spectra(MLINK link) const = 0;

   virtual const Spectrum_generator_problems& get_problems() const = 0;
   virtual void fill_slha_io(ScalarSingletZ2DMEFTHiggs_slha_io&) const = 0;
   virtual double get_model_scale() const = 0;
   virtual const ScalarSingletZ2DMEFTHiggs_observables& get_observables() const = 0;

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const ScalarSingletZ2DMEFTHiggs_input_parameters&) = 0;
   virtual void calculate_model_observables(const softsusy::QedQcd&, const Physical_input&) = 0;
};

template <typename Solver_type>
class ScalarSingletZ2DMEFTHiggs_spectrum_impl : public ScalarSingletZ2DMEFTHiggs_spectrum
{
public:
   virtual ~ScalarSingletZ2DMEFTHiggs_spectrum_impl() = default;
   ...
   virtual void fill_slha_io(ScalarSingletZ2DMEFTHiggs_slha_io&) const override;
   virtual double get_model_scale() const override { return std::get<0>(models).get_scale(); }
   virtual const ScalarSingletZ2DMEFTHiggs_observables& get_observables() const override { return observables; }

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const ScalarSingletZ2DMEFTHiggs_input_parameters&) override;
   virtual void calculate_model_observables(const softsusy::QedQcd&, const Physical_input&) override;

private:
   std::tuple<ScalarSingletZ2DMEFTHiggs<Solver_type>, standard_model::StandardModel<Two_scale>> models{};        ///< running parameters and pole masses
   Spectrum_generator_problems problems{};   ///< spectrum generator problems
   ScalarSingletZ2DMEFTHiggs_scales scales{};              ///< scale information
   ScalarSingletZ2DMEFTHiggs_observables observables{};    ///< observables
};
...
public:
   ...
   void set_input_parameters(const ScalarSingletZ2DMEFTHiggs_input_parameters& input_) { input = input_; }
...
private:
   ScalarSingletZ2DMEFTHiggs_input_parameters input{};     ///< model input parameters
   ...
   std::unique_ptr<ScalarSingletZ2DMEFTHiggs_spectrum> spectrum{nullptr};  ///< spectrum information

   ScalarSingletZ2DMEFTHiggs_slha_io get_slha_io() const;
...
void Model_data::check_spectrum_pointer() const
{
   if (!spectrum) {
      throw SetupError("No spectrum generator set! "
                       "Did you run FSScalarSingletZ2DMEFTHiggsCalculateSpectrum[]?");
   }
}
                ...
                const std::vector<std::string>& heads = {})
{
   for (std::size_t i = 0; i < flags.size(); i++) {
      if (flags[i]) {
         ...
         MLPutSymbol(link, ScalarSingletZ2DMEFTHiggs_info::particle_names[i].c_str());
      }
   }
}
...
ScalarSingletZ2DMEFTHiggs_slha_io Model_data::get_slha_io() const
{
   check_spectrum_pointer();
   ScalarSingletZ2DMEFTHiggs_slha_io slha_io;
...
void put_spectrum(const ScalarSingletZ2DMEFTHiggs_slha& model, MLINK link)
{
   MLPutRule(link, ScalarSingletZ2DMEFTHiggs_info::model_name);
...
template <typename Solver_type>
void ScalarSingletZ2DMEFTHiggs_spectrum_impl<Solver_type>::put_model_spectra(MLINK link) const
{
...
template <typename Solver_type>
void ScalarSingletZ2DMEFTHiggs_spectrum_impl<Solver_type>::calculate_spectrum(
...
{
   ScalarSingletZ2DMEFTHiggs_spectrum_generator<Solver_type> spectrum_generator;
...
template <typename Solver_type>
void ScalarSingletZ2DMEFTHiggs_spectrum_impl<Solver_type>::calculate_model_observables(
...
template <typename Solver_type>
void ScalarSingletZ2DMEFTHiggs_spectrum_impl<Solver_type>::fill_slha_io(ScalarSingletZ2DMEFTHiggs_slha_io& slha_io) const
{
...
{
   check_spectrum_pointer();
   ScalarSingletZ2DMEFTHiggs_observables observables = spectrum->get_observables();
...
void Model_data::check_spectrum(MLINK link) const
{
   ...
   for (const auto& s: problems.get_problem_strings())
      put_message(link, "FSScalarSingletZ2DMEFTHiggsCalculateSpectrum", "error", s);

   for (const auto& s: problems.get_warning_strings())
      put_message(link, "FSScalarSingletZ2DMEFTHiggsCalculateSpectrum", "warning", s);
...
#ifdef ENABLE_TWO_SCALE_SOLVER
   case 1:
      spectrum.reset(new ScalarSingletZ2DMEFTHiggs_spectrum_impl<Two_scale>());
...
DLLEXPORT int FSScalarSingletZ2DMEFTHiggsGetSettings(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::ScalarSingletZ2DMEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSScalarSingletZ2DMEFTHiggsGetSettings"))
...
DLLEXPORT int FSScalarSingletZ2DMEFTHiggsGetSMInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::ScalarSingletZ2DMEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSScalarSingletZ2DMEFTHiggsGetSMInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSScalarSingletZ2DMEFTHiggsGetInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::ScalarSingletZ2DMEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSScalarSingletZ2DMEFTHiggsGetInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSScalarSingletZ2DMEFTHiggsOpenHandle(
...
DLLEXPORT int FSScalarSingletZ2DMEFTHiggsCloseHandle(
   WolframLibraryData /* libData */, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy::ScalarSingletZ2DMEFTHiggs_librarylink;
...
DLLEXPORT int FSScalarSingletZ2DMEFTHiggsSet(
   WolframLibraryData libData, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy;
   using namespace flexiblesusy::ScalarSingletZ2DMEFTHiggs_librarylink;
...
DLLEXPORT int FSScalarSingletZ2DMEFTHiggsGetProblems(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::ScalarSingletZ2DMEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSScalarSingletZ2DMEFTHiggsGetProblems"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSScalarSingletZ2DMEFTHiggsToSLHA(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::ScalarSingletZ2DMEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSScalarSingletZ2DMEFTHiggsToSLHA"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSScalarSingletZ2DMEFTHiggsGetWarnings(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::ScalarSingletZ2DMEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSScalarSingletZ2DMEFTHiggsGetWarnings"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSScalarSingletZ2DMEFTHiggsCalculateSpectrum(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::ScalarSingletZ2DMEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSScalarSingletZ2DMEFTHiggsCalculateSpectrum"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSScalarSingletZ2DMEFTHiggsCalculateObservables(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::ScalarSingletZ2DMEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSScalarSingletZ2DMEFTHiggsCalculateObservables"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSScalarSingletZ2DMEFTHiggsCalculateObservables", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSScalarSingletZ2DMEFTHiggsCalculateSpectrum[]?");
      }
      ...
      put_message(link, "FSScalarSingletZ2DMEFTHiggsCalculateObservables", "error", e.what_detailed());
      put_error_output(link);
   }
