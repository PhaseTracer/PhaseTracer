...
double ScalarSingletZ2DM_spectrum_generator<Two_scale>::get_pole_mass_scale(double susy_scale) const
{
...
double ScalarSingletZ2DM_spectrum_generator<Two_scale>::get_eft_pole_mass_scale(double susy_scale, double Mt) const
{
 ...
 */
void ScalarSingletZ2DM_spectrum_generator<Two_scale>::run_except(const softsusy::QedQcd& qedqcd,
                                const ScalarSingletZ2DM_input_parameters& input)
{
   ...
   ScalarSingletZ2DM_ewsb_solver<Two_scale> ewsb_solver;
   model.set_ewsb_solver(
      std::make_shared<ScalarSingletZ2DM_ewsb_solver<Two_scale> >(ewsb_solver));

   ScalarSingletZ2DM_high_scale_constraint<Two_scale> high_scale_constraint(&model);
   ScalarSingletZ2DM_susy_scale_constraint<Two_scale> susy_scale_constraint(&model, qedqcd);
   ...
   ScalarSingletZ2DM_standard_model_matching_down<Two_scale> matching_down(
   ...
   // convergence tester for ScalarSingletZ2DM
   ScalarSingletZ2DM_convergence_tester<Two_scale> model_ct(
   ...
   ScalarSingletZ2DM_standard_model_initial_guesser<Two_scale> initial_guesser(&model, &eft, qedqcd,
 ...
 */
void ScalarSingletZ2DM_spectrum_generator<Two_scale>::write_running_couplings(
   const std::string& filename) const
{
   ScalarSingletZ2DM_spectrum_generator_interface<Two_scale>::write_running_couplings(
      filename, get_low_scale(), get_high_scale());
}

void ScalarSingletZ2DM_spectrum_generator<Two_scale>::calculate_spectrum(double Mt, double MW)
{
   ...
   if (eft.get_problems().is_running_tachyon(standard_model_info::hh))
      model.get_problems().flag_running_tachyon(ScalarSingletZ2DM_info::hh);
   if (eft.get_problems().is_pole_tachyon(standard_model_info::hh))
      model.get_problems().flag_pole_tachyon(ScalarSingletZ2DM_info::hh);
   if (eft.get_problems().is_running_tachyon(standard_model_info::VZ))
      model.get_problems().flag_running_tachyon(ScalarSingletZ2DM_info::VZ);
   if (eft.get_problems().is_pole_tachyon(standard_model_info::VZ))
      model.get_problems().flag_pole_tachyon(ScalarSingletZ2DM_info::VZ);
   if (eft.get_problems().is_running_tachyon(standard_model_info::VWp))
      model.get_problems().flag_running_tachyon(ScalarSingletZ2DM_info::VWp);
   if (eft.get_problems().is_pole_tachyon(standard_model_info::VWp))
      model.get_problems().flag_pole_tachyon(ScalarSingletZ2DM_info::VWp);
}
