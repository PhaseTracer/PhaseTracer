...
#include "ScalarSingletZ2DM_edm.hpp"
#include "ScalarSingletZ2DM_l_to_lgamma.hpp"
//#include "ScalarSingletZ2DM_f_to_f_conversion.hpp"
...
const int ScalarSingletZ2DM_observables::NUMBER_OF_OBSERVABLES;

ScalarSingletZ2DM_observables::ScalarSingletZ2DM_observables()
   : a_muon(0)
...
Eigen::ArrayXd ScalarSingletZ2DM_observables::get() const
{
   Eigen::ArrayXd vec(ScalarSingletZ2DM_observables::NUMBER_OF_OBSERVABLES);
...
std::vector<std::string> ScalarSingletZ2DM_observables::get_names()
{
   std::vector<std::string> names(ScalarSingletZ2DM_observables::NUMBER_OF_OBSERVABLES);
...
void ScalarSingletZ2DM_observables::clear()
{
   a_muon = 0.;
...
void ScalarSingletZ2DM_observables::set(const Eigen::ArrayXd& vec)
{
   assert(vec.rows() == ScalarSingletZ2DM_observables::NUMBER_OF_OBSERVABLES);
...
ScalarSingletZ2DM_observables calculate_observables(ScalarSingletZ2DM_mass_eigenstates& model,
                                              ...
                                              double scale)
{
   ...
   if (scale > 0.) {
      try {
         ...
         return ScalarSingletZ2DM_observables();
      }
   }
                                              ...
                                              const Physical_input& physical_input)
{
   ...
   try {
      ScalarSingletZ2DM_effective_couplings effective_couplings(model, qedqcd, physical_input);
      effective_couplings.calculate_effective_couplings();

      observables.AMU = ScalarSingletZ2DM_a_muon::calculate_a_muon(MODEL, qedqcd);
