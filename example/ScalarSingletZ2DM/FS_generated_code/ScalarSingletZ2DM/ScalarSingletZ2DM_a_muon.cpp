...
double get_MSUSY(const ScalarSingletZ2DM_mass_eigenstates& model)
{
   return 0.;
}

void run_to_MSUSY(ScalarSingletZ2DM_mass_eigenstates& model)
{
   ...
   VERBOSE_MSG("ScalarSingletZ2DM_a_muon: iterate to run to MSUSY ...");
   ...
   do {
      ...
      VERBOSE_MSG("ScalarSingletZ2DM_a_muon:    running to new MSUSY = " << MSUSY_new);
   ...
   VERBOSE_MSG("ScalarSingletZ2DM_a_muon: iteration finished. MSUSY = " << MSUSY_new);

   if (precision > precision_goal) {
      WARNING("ScalarSingletZ2DM_a_muon: run_to_MSUSY(): "
...
double calculate_a_muon_impl(const ScalarSingletZ2DM_mass_eigenstates& model, const softsusy::QedQcd& qedqcd)
{
   VERBOSE_MSG("ScalarSingletZ2DM_a_muon: calculating a_mu at Q = " << model.get_scale());

   context_base context{ model };
   
   using namespace ScalarSingletZ2DM_cxx_diagrams::fields;

   const auto form_factors = ScalarSingletZ2DM_FFV_form_factors::calculate_Fe_Fe_VP_form_factors(1, 1, model, true);
...
std::pair<double,double> vary_scale(const ScalarSingletZ2DM_mass_eigenstates& model, const softsusy::QedQcd& qedqcd)
{
   ...
   std::transform(scales.begin(), scales.end(), scales.begin(),
                  [&model,&qedqcd] (double scale) {
                     ...
                     try {
                        ...
                        ERROR("ScalarSingletZ2DM_a_muon: scale variation: " << e.what_detailed());
                     }
                     return amu;
                  });
...
double ScalarSingletZ2DM_a_muon::calculate_a_muon(const ScalarSingletZ2DM_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd)
{
   ScalarSingletZ2DM_mass_eigenstates model(model_);

   VERBOSE_MSG("ScalarSingletZ2DM_a_muon: starting calculation of a_mu ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("ScalarSingletZ2DM_a_muon:" << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
...
double ScalarSingletZ2DM_a_muon::calculate_a_muon_uncertainty(const ScalarSingletZ2DM_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd)
{
   ScalarSingletZ2DM_mass_eigenstates model(model_);

   VERBOSE_MSG("ScalarSingletZ2DM_a_muon: starting calculation of a_mu uncertainty ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("ScalarSingletZ2DM_a_muon uncertainty: " << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
