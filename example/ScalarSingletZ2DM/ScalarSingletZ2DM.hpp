...
typedef fs::ScalarSingletZ2DM_spectrum_generator<fs::Two_scale> SpectrumGenerator;
typedef fs::ScalarSingletZ2DM_slha<fs::ScalarSingletZ2DM<fs::Two_scale>> Model;
typedef fs::ScalarSingletZ2DM_input_parameters Input;
typedef fs::Spectrum_generator_settings Settings;


namespace EffectivePotential {

class ScalarSingletZ2DM : public OneLoopPotential {
 public:
  ScalarSingletZ2DM() {
...
void ScalarSingletZ2DM::set_input(std::vector<double> x) {
  Input input;
  input.Qin = x[0];
  input.QEWSB = x[1];
  input.muH2Input = x[2];
...
double ScalarSingletZ2DM::V0(Eigen::VectorXd phi) const {
...
std::vector<double> ScalarSingletZ2DM::get_scalar_debye_sq(Eigen::VectorXd phi, double xi, double T) const {
...
std::vector<double> ScalarSingletZ2DM::get_vector_debye_sq(Eigen::VectorXd phi, double T) const {
...
std::vector<double> ScalarSingletZ2DM::get_scalar_masses_sq(Eigen::VectorXd phi, double xi) const {
  return get_scalar_debye_sq(phi, xi, 0.);
}

std::vector<double> ScalarSingletZ2DM::get_vector_masses_sq(Eigen::VectorXd phi) const {
  return get_vector_debye_sq(phi, 0.);
}

std::vector<double> ScalarSingletZ2DM::get_fermion_masses_sq(Eigen::VectorXd phi) const {
  auto model_copy = model;
  model_copy.solve_ewsb_tree_level();
  model_copy.set_v(phi[0]);
