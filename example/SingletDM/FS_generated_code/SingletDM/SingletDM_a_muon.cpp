...
double calculate_a_muon_impl(const SingletDM_mass_eigenstates& model, const softsusy::QedQcd& qedqcd)
{
   ...
   const auto form_factors = SingletDM_FFV_form_factors::calculate_Fe_Fe_VP_form_factors(1, 1, model, true);
...
std::pair<double,double> vary_scale(const SingletDM_mass_eigenstates& model, const softsusy::QedQcd& qedqcd)
{
   ...
   std::transform(scales.begin(), scales.end(), scales.begin(),
                  [&model,&qedqcd] (double scale) {
                     ...
                     try {
                        ...
                        ERROR("SingletDM_a_muon: scale variation: " << e.what_detailed());
                     }
                     return amu;
                  });
...
double SingletDM_a_muon::calculate_a_muon(const SingletDM_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd)
{
   ...
   try {
      ...
      ERROR("SingletDM_a_muon:" << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
...
double SingletDM_a_muon::calculate_a_muon_uncertainty(const SingletDM_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd)
{
   SingletDM_mass_eigenstates model(model_);
   ...
   try {
      ...
      ERROR("SingletDM_a_muon uncertainty: " << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
