...
int main(int argc, char* argv[]) {
  ...
  // default output
  std::string output_filename = "z_eos_scan/msbar_output_new.csv";
  ...
  try {
    wallVelocities = wallGoWrapper::getWallVelocity(valid_t[0], params4d, tperc, "msbar_xSM");
  } catch (...) { wallVelocities = wallVelocities;}
  for ( auto v : wallVelocities ) { std::cout << "Wall velocity from wallgo: " << v << std::endl; }
  if ( wallVelocities.size() == 1) {
    vw = wallVelocities[0];
    vwLTE = -1;
  } else {
    vw = vw;
  ...
  // const auto eos_path = "z_eos_scan/eos/msbar_l_hs_" + std::to_string(lambda_hs) + "_l_s_" + std::to_string(lambda_s) + "_m_s_" + std::to_string(mS) + ".csv";
  // const auto eos_path = "z_eos_scan/msbar_bp1.csv";
