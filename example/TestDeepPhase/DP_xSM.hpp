...
/*
  Format output
*/
std::string formatOutput( std::vector<double> in, std::vector<double> out ) {
  std::string output = "";
  for ( double val : in ) { output += std::to_string(val) + ","; }
  for ( double val : out ) { output += std::to_string(val) + ","; }
...
/*
  For the WallGo interface
*/
std::string execPythonScript(const std::string& cmd) {
...
/*
  Stores the WallGo input information so python can access it
*/
std::string writeJsonToTempFile(const json& j) {
  // Template for mkstemp: XXXXXX will be replaced with random chars
  std::string baseTemplate = "/tmp/inputXXXXXX";
  std::vector<char> tempName(baseTemplate.begin(), baseTemplate.end());
  tempName.push_back('\0');

  int fd = mkstemp(tempName.data());
  if (fd == -1) throw std::runtime_error("Failed to create temp file");

  std::string tempFilePath = std::string(tempName.data()) + ".json";
  rename(tempName.data(), tempFilePath.c_str());  // add .json extension

  std::ofstream out(tempName.data());
  if (!out.is_open()) throw std::runtime_error("Failed to open temp file stream");
...
/*
  Deletes the WallGo input file after evaluation
*/
void safeDeleteFile(const std::string& path) {
