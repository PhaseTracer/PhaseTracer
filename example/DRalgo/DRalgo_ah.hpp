...
//    std::cout << std::endl << "veffLO=" << veffLO << "veffNLO=" << veffNLO << "veffNNLO=" << veffNNLO << std::endl;
  ...
  void solveBetas(std::vector<double> x0){
    // Generated by ChatGPT
    double dt = 1;                        // Scale step
    double t_start = 100.0;               // Start scale
    double t_end = 5000.0;                // End scale
    // Define the type for odeint
    using state_type = std::vector<double>;
    // Define the state variables
    state_type x = x0;
    // Define the stepper
    boost::numeric::odeint::runge_kutta_dopri5<state_type> stepper;    // Solve the ODEs and print the results
    std::vector<double> t_vec;   // Store the values of t
    std::vector<state_type> x_vec;  // Store the values of x
    for (double t = t_start; t <= t_end; t += dt)
    {
        stepper.do_step(Betas, x, t, dt);  // Solve the ODEs
        t_vec.push_back(t);
        x_vec.push_back(x);
    }
