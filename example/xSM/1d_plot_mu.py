...
def plot_for_gauge(data_MS, color):
  ...
  gamma_norm = 1
  if norm :
    mu_loc = abs(mu-173) < 0.05
#    print len(xi[xi_loc])
#    print xi[xi_loc]
    TC_norm = TC_MS[mu_loc]
    gamma_norm = gamma_MS[mu_loc]
    
  ax = axs[0]
  ax.plot(mu[sel], TC_MS[sel]/TC_norm,  c=color, alpha=1)

  ax = axs[1]
  ax.plot(mu[sel], gamma_MS[sel]/gamma_norm,  c=color, alpha=1)


plot_for_gauge(np.loadtxt("goldstone_catastrophe/BK1_mu.txt"),  "r")
plot_for_gauge(np.loadtxt("goldstone_catastrophe/BK2_mu.txt"),  "orange")
plot_for_gauge(np.loadtxt("goldstone_catastrophe/BK3_mu.txt"),  "g")
plot_for_gauge(np.loadtxt("goldstone_catastrophe/BK4_mu.txt"),  "cyan")
#plot_for_gauge(np.loadtxt("goldstone_catastrophe/BK5_mu.txt"),  "b")
plot_for_gauge(np.loadtxt("goldstone_catastrophe/BK6_mu.txt"),  "purple")
...
fig.tight_layout()
plt.savefig('mu_1d'+('_norm' if norm else '' )+'.png')
