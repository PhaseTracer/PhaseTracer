...
def range_for_1d(data1, data2, x_num, label, column, color):
  ...
  x_TC = np.linspace(min(min(x_TC1),min(x_TC2)), max(max(x_TC1),max(x_TC2)), num=50, endpoint=True)
  x_gamma = np.linspace(min(min(x_gamma1),min(x_gamma2)), max(max(x_gamma1),max(x_gamma2)), num=50, endpoint=True)
