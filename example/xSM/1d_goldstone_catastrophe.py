...
def plot_for_gauge(data_MS, color, label):
...
#    print xi[xi_loc]
  ...
  ax = axs[0]
  ax.plot(xi[sel], TC_MS[sel]/TC_norm, ('.' if use_dot else '-'), c=color, label = label, alpha=1)

  ax = axs[1]
  ax.plot(xi[sel], gamma_MS[sel]/gamma_norm, ('.' if use_dot else '-'), c=color, label = label, alpha=1)

plot_for_gauge(np.loadtxt("goldstone_catastrophe/BK2_no_gildstone_catastrophe.txt"),  "k", "No treatment")
plot_for_gauge(np.loadtxt("goldstone_catastrophe/BK2_goldstone_resum.txt"),  "r", "Goldstone resum")
plot_for_gauge(np.loadtxt("goldstone_catastrophe/BK2_use_1L_EWSB_in_0L_mass.txt"),  "orange", "Use 1L tadpole in 0L mass")

plot_for_gauge(np.loadtxt("goldstone_catastrophe/BK2_both.txt"),  "b", "both")
...
fig.tight_layout()
plt.savefig('goldstone_catastrophe'+('_norm' if norm else '' )+'.png')
