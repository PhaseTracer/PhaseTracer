...
class ScalarSingletZ2DMMhInput : public OneLoopPotential {
 public:
  ScalarSingletZ2DMMhInput() {
...
void ScalarSingletZ2DMMhInput::set_input(std::vector<double> x) {
  Input input;
  input.Qin = x[0];
  input.QEWSB = x[1];
  input.MhInput = x[2];
...
std::vector<double> ScalarSingletZ2DMMhInput::get_scalar_debye_sq(Eigen::VectorXd phi, double xi, double T) const {
...
std::vector<double> ScalarSingletZ2DMMhInput::get_vector_debye_sq(Eigen::VectorXd phi, double T) const {
...
std::vector<double> ScalarSingletZ2DMMhInput::get_scalar_masses_sq(Eigen::VectorXd phi, double xi) const {
  return get_scalar_debye_sq(phi, xi, 0.);
}

std::vector<double> ScalarSingletZ2DMMhInput::get_vector_masses_sq(Eigen::VectorXd phi) const {
  return get_vector_debye_sq(phi, 0.);
}

std::vector<double> ScalarSingletZ2DMMhInput::get_fermion_masses_sq(Eigen::VectorXd phi) const {
  auto model_copy = model;
  model_copy.solve_ewsb_tree_level();
  model_copy.set_v(phi[0]);
