...
public:
  xSM_MSbar_parameters_solver(double ms_,
                              ...
                              double b3_,
                              double vs_,
                              double lambda_s_) : ms(ms_), c1(c1_), c2(c2_), b3(b3_), vs(vs_), lambda_s(lambda_s_) {
    vev << v, vs;
    Z2_symmetry = vs == 0;
  }
...
private:
  ...
  bool DetermineParametersAtTreeLevel() {
    ...
    if (not Z2_symmetry) {
      ...
      mu_s_Sq_tree = -2 * lambda_s_tree * square(vs) - 0.5 * c1 * square(v) / vs - c2 * square(v) - 1.5 * b3 * vs;
    } else {
  ...
  bool DetermineParametersAtOneLoop() {
    ...
    mu_h_Sq_iteration = -0.25 * mhh - 0.75 / v * dVCWdh_ + 0.25 * d2VCWdh2_ - c1 * vs - c2 * vs * vs;
    if (not Z2_symmetry) {
      lambda_s_iteration = (mss + c1 * v * v / vs - 3 * b3 * vs + 1 / vs * dVCWds_ - d2VCWds2_) / (8. * vs * vs);
      mu_s_Sq_iteration = -0.25 * mss - 0.75 * (c1 * v * v / vs + b3 * vs + dVCWds_ / vs - d2VCWds2_ / 3.) - c2 * v * v;
    } else {
    ...
    if (not Z2_symmetry) {
      mu_s_Sq_tree_EWSB = -1. / (2. * vs) * (4. * lambda_s_iteration * vs * vs * vs + c1 * v * v + 2. * c2 * v * v * vs + 3. * b3 * vs * vs);
    } else {
  ...
  std::vector<double> d2_massEigenvalues_dh2(Eigen::VectorXd phi) {
    ...
    const double b = (((d2mhhdh2 - d2mssdh2) * (mhh - mss) + square(dmhhdh - dmssdh) + 4 * d2mhsdh2 * mhs + 4 * square(dmhsdh)) * (square(mhh - mss) + 4 * mhs) - square((dmhhdh - dmssdh) * (mhh - mss) + 4 * dmhsdh * mhs)) / pow((square(mhh - mss) + 4 * square(mhs)), 1.5);
    return {0.5 * (a + b), 0.5 * (a - b)};
  }
  ...
  std::vector<double> d2_massEigenvalues_ds2(Eigen::VectorXd phi) {
    ...
    const double b = (((d2mhhds2 - d2mssds2) * (mhh - mss) + square(dmhhds - dmssds) + 4 * d2mhsds2 * mhs + 4 * square(dmhsds)) * (square(mhh - mss) + 4 * mhs) - square((dmhhds - dmssds) * (mhh - mss) + 4 * dmhsds * mhs)) / pow((square(mhh - mss) + 4 * square(mhs)), 1.5);
    return {0.5 * (a + b), 0.5 * (a - b)};
  }
  ...
  std::vector<double> d2_massEigenvalues_dhds(Eigen::VectorXd phi) {
    ...
    const double b = (((d2mhhdhds - d2mssdhds) * (mhh - mss) + (dmhhdh - dmssdh) * (dmhhds - dmssds) + 4 * d2mhsdhds * mhs + 4 * dmhsdh * dmhsds) * (square(mhh - mss) + 4 * mhs) - ((dmhhdh - dmssdh) * (mhh - mss) + 4 * dmhsdh * mhs) * ((dmhhds - dmssds) * (mhh - mss) + 4 * dmhsds * mhs)) / pow((square(mhh - mss) + 4 * square(mhs)), 1.5);
  ...
  ****************************************** */
  double dVCWdh(Eigen::VectorXd phi) {
    ...
    for (size_t i = 0; i < mb.size(); ++i) {
      y += boson_dofs[i] * mb[i] * dmbdh[i] * (logx(std::abs(mb[i] / square(Q)) + zero) - boson_cs[i] + 0.5);
    }

    const auto mf = fermion_massSq(phi);
    const auto dmfdh = d_fermion_massSq_dh(phi);
    for (size_t i = 0; i < mf.size(); ++i) {
      y -= fermion_dofs[i] * mf[i] * dmfdh[i] * (logx(std::abs(mf[i] / square(Q)) + zero) - 1.5 + 0.5);
    }
  ...
  double dVCWds(Eigen::VectorXd phi) {
    ...
    double y = 0;
    for (size_t i = 0; i < mb.size(); ++i) {
      y += boson_dofs[i] * mb[i] * dmbds[i] * (logx(std::abs(mb[i] / square(Q)) + zero) - boson_cs[i] + 0.5);
    }

    const auto mf = fermion_massSq(phi);
    const auto dmfds = d_fermion_massSq_ds(phi);
    for (size_t i = 0; i < mf.size(); ++i) {
      y -= fermion_dofs[i] * mf[i] * dmfds[i] * (logx(std::abs(mf[i] / square(Q)) + zero) - 1.5 + 0.5);
    }
  ...
  double VCW(Eigen::VectorXd phi) {
    ...
    for (size_t i = 0; i < mb.size(); ++i) {
      //      if (i<=3) continue;
      y += boson_dofs[i] * mb[i] * mb[i] * (logx(mb[i] / square(Q)) - boson_cs[i]);
    }

    const auto mf = fermion_massSq(phi);
    for (size_t i = 0; i < mf.size(); ++i) {
      y -= fermion_dofs[i] * mf[i] * mf[i] * (logx(mf[i] / square(Q)) - 1.5);
    }
  ...
  double d2VCWds2_test(Eigen::VectorXd phi) {
    ...
    for (size_t i = 0; i < mb.size(); ++i) {
      if (i <= 4)
        continue;
      y += boson_dofs[i] * ((square(dmbds[i]) + mb[i] * d2mbds2[i]) * (logx(std::abs(mb[i] / square(Q)) + zero) - boson_cs[i] + 0.5) + square(dmbds[i]));
    }
  ...
  double d2VCWdh2(Eigen::VectorXd phi) {
    ...
    double y = 0;
    for (size_t i = 0; i < mb.size(); ++i) {
      y += boson_dofs[i] * ((square(dmbdh[i]) + mb[i] * d2mbdh2[i]) * (logx(std::abs(mb[i] / square(Q)) + zero) - boson_cs[i] + 0.5) + square(dmbdh[i]));
      //      std::cout << "!!!!!-----! y=" <<
    ...
    for (size_t i = 0; i < mf.size(); ++i) {
      y -= fermion_dofs[i] * ((square(dmfdh[i]) + mf[i] * d2mfdh2[i]) * (logx(std::abs(mf[i] / square(Q)) + zero) - 1.5 + 0.5) + square(dmfdh[i]));
  ...
  double d2VCWds2(Eigen::VectorXd phi) {
    ...
    const auto d2mfds2 = d2_fermion_massSq_ds2(phi);
    for (size_t i = 0; i < mf.size(); ++i) {
      y -= fermion_dofs[i] * ((square(dmfds[i]) + mf[i] * d2mfds2[i]) * (logx(std::abs(mf[i] / square(Q)) + zero) - 1.5 + 0.5) + square(dmfds[i]));
    }
    return y / (32 * M_PI * M_PI);
  }
  double d2VCWdhds(Eigen::VectorXd phi) {
    ...
    double y = 0;
    for (size_t i = 0; i < mb.size(); ++i) {
      y += boson_dofs[i] * ((dmbdh[i] * dmbds[i] + mb[i] * d2mbdhds[i]) * (logx(std::abs(mb[i] / square(Q)) + zero) - boson_cs[i] + 0.5) + dmbdh[i] * dmbds[i]);
    }
    ...
    for (size_t i = 0; i < mf.size(); ++i) {
      y -= fermion_dofs[i] * ((dmfds[i] * dmfdh[i] + mf[i] * d2mfdhds[i]) * (logx(std::abs(mf[i] / square(Q)) + zero) - 1.5 + 0.5) + dmfds[i] * dmfdh[i]);
    }
    return y / (32 * M_PI * M_PI);
  }
