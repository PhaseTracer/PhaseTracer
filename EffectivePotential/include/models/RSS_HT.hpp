...
class RSS_HT : public RSS
{
	...
	*/
	...
	void setParameters(double muh_sq_, double mus_sq_, double lh_, double ls_, double c1_, double c2_, double b3_, double
		vs_, double ms_, double theta_)
	{
		muh_sq = muh_sq_;
		mus_sq = mus_sq_;
		lh = lh_;
	...
	void setParameters(std::vector<double> params) override
	{
		if(params.size() != 10)
		{
			std::cerr << "Called RSS_HT::setParameters with the wrong number of parameters: got " <<
				params.size() << ", expected 10." << std::endl;
			return;
		}
	...
	double V(Eigen::VectorXd phi, double T) const override
	{
		...
		return (muh_sq + 0.5*debye[0])*h2 + (mus_sq + 0.5*debye[1])*s2 + lh*h2*h2 + ls*s2*s2 + c1*h2*s + c2*h2*s2
			+ b3*s2*s;
	}
}; // class RSS_HT
}  // namespace EffectivePotential
