...
public:
  ...
  double V(Eigen::VectorXd phi, double T) const override {
    ...
    return 0.2 + 0.5 * T - 0.3 * T2 + (-4.9588 - 3.8262 * T + 2.3338 * T2) * x + (10.9473 + 8.94145 * T - 6.23755 * T2) * x2 + (-11.4397 - 8.92383 * T + 6.89683 * T2) * x3 + (6.1325 + 4.27625 * T - 3.60875 * T2) * x4 + (-1.62 - 0.97 * T + 0.89 * T2) * x5 + (0.166667 + 0.0833333 * T - 0.0833333 * T2) * x6;
  }
