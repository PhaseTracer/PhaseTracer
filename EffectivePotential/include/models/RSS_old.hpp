...
class RSS : public OneLoopPotential
{
	...
	*/
	RSS(double muh_sq_, double mus_sq_, double lh_, double ls_, double c1_, double c2_, double b3_, double
	...
	virtual void setParameters(std::vector<double> params)
	{
		if(params.size() != 12)
		{
			std::cerr << "Called RSS::setParameters with the wrong number of parameters: got " <<
				params.size() << ", expected 12." << std::endl;
			return;
		}
