...
public:
  ...
  std::vector<double> get_scalar_debye_sq(Eigen::VectorXd phi, double xi, double T) const override {
    ...
    const double sum = 1. / (16. * M_PI * M_PI) * (3. * lambda_h * (Qsq * xlogx(mhh2 / Qsq) - mhh2) + 0.5 * lambda_hs * (Qsq * xlogx(mss2 / Qsq) - mss2) - 6. * SM_yt_sq * (Qsq * xlogx(fm2[0] / Qsq) - fm2[0]) - 6. * SM_yb_sq * (Qsq * xlogx(fm2[1] / Qsq) - fm2[1]) // TODO: Need check
