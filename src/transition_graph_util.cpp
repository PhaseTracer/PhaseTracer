...
/**
  ...
  */
PhaseTracer::Phase constructSymmetricPartnerPhase(const EffectivePotential::Potential& model, const PhaseTracer::Phase&
	phase, const std::vector<int>& reflectionIndices, int key)
{
			...
			if(needToClonePath)
			{
				...
				if(phaseStructureData.isLowTemperaturePhase[frontier[i].vertexIndex])
				{
					...
					for(int j = 0; j < terminalSubpaths.size(); ++j)
					{
						...
						// not duplicate paths.
						if(terminalSubpaths[j].phases.size() != paths[frontier[i].pathIndex].phases.size()-1
							|| terminalSubpaths[j].phases.back() != paths[frontier[i].pathIndex].phases.end()[-2])
						{
							continue;
						}
