...
void Shooting::exactSolution(double r, double phi0, double dV_, double d2V_,
  ...
  if (beta_r < 1e-2) {
    ...
    try {
      ...
      dphi += pow(0.5 * beta_r, -nu) * 0.5 * beta * (boost::math::cyl_bessel_i(nu - 1, beta_r) + boost::math::cyl_bessel_i(nu + 1, beta_r));
    ...
    dphi += pow(0.5 * beta_r, -nu) * 0.5 * beta * (boost::math::cyl_bessel_j(nu - 1, beta_r) - boost::math::cyl_bessel_j(nu + 1, beta_r));
    dphi *= std::tgamma(nu + 1) * dV_ / d2V_;
    phi += phi0;
  }
...
double Shooting::calAction(Profile1D profile) {
  ...
  if (r.size() == 2) {
    if (r.isApprox(profile_zero.R, xtol))
      return 0;
    if (r.isApprox(profile_inf.R, xtol))
